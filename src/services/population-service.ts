...
export class PopulationServiceImpl implements PopulationService {
  ...
  constructor(baseUrl: string) {
    ...
    this.countriesApiBaseUrl = `${cleanBaseUrl}/${WorldBankApiV2.VERSION}/${
  ...
  getBaseIndicatorApiUrlFor(
    ...
    return `${this.countriesApiBaseUrl}/${countryCode}/${
      WorldBankApiV2.INDICATORS_API_PREFIX
    }/${indicator}`;
  }
  ...
  async getIndicatorData(
    ...
    if (dataPoints) {
      retVal = dataPoints
        .filter(dataPoint => dataPoint.val !== null)
        .map(
          dataPoint => new DataPoint(dataPoint.date, dataPoint.value as number)
        );
    }
    return retVal;
  }
  ...
  async getAllCountries(): Promise<Country[]> {
    const response: Response = await fetch(
      `${this.countriesApiBaseUrl}?${WorldBankApiV2Params.FORMAT}=${
        WorldBankApiV2Formats.JSON
    ...
    // ThrowReporter.report(validationResult);
    console.log("Reponse recieved and validated");
    const countries = (validationResult.left[0]
      .value as WorldBankApiV2CountryResponse)[1];
  ...
  async getCountry(countryCode: string): Promise<Country> {
    ...
    const response: Response = await fetch(
      `${this.countriesApiBaseUrl}/${countryCode}?${
        WorldBankApiV2Params.FORMAT
      }=${WorldBankApiV2Formats.JSON}`
    );
    ...
    if (countries.length > 1) {
      return Promise.reject(
        "More than on country was returned. This should not happen"
      );
    }
  ...
  async getTotalPopulation(
    ...
    // ThrowReporter.report(validationResult);
    const dataPoints = (validationResult.left as WorldBankApiV2IndicatorResponse)[1];
    let retVal: DataPoint[] = [];
    if (dataPoints) {
      retVal = dataPoints
