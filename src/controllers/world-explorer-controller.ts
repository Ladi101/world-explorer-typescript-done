import { WorldExplorerController } from "./world-explorer-controller.in";
...
export class WorldExplorerControllerImpl implements WorldExplorerController {
  ...
  constructor(populationService: PopulationService, view: WorldExplorerView) {
    ...
    this._view = view;
    this.loadCountries()
      .then(() => console.log("WorlExplorer - Loaded", this))
      .catch(error => {
        console.error(
          "WorldExplorer - Controller failed to load. Error: ",
          error
        );
  ...
  async renderChart(): Promise<void> {
    ...
    if (this._countriesMap.has(chartFormDetails.countryId)) {
      ...
      console.error(
        "Tried to render a chart for an unknown country. Identifier: ",
        chartFormDetails.countryId
      );
    ...
    try {
      ...
      switch (indicator) {
        case WorldBankApiV2Indicators.TOTAL_POPULATION:
          data = await this._populationService.getTotalPopulation(
            country,
            dateRange
          );
        ...
        case WorldBankApiV2Indicators.TOTAL_FEMALE_POPULATION:
          data = await this._populationService.getFemalePopulation(
            country,
            dateRange
          );
          title = "Title female population";
          break;
        case WorldBankApiV2Indicators.TOTAL_MALE_POPULATION:
          data = await this._populationService.getMalePopulation(
            country,
            dateRange
          );
        ...
        case WorldBankApiV2Indicators.ADULT_MALE_LITERACY:
          data = await this._populationService.getAdultMaleLiteracy(
            country,
            dateRange
          );
        ...
        case WorldBankApiV2Indicators.ADULT_FEMALE_LITERACY:
          data = await this._populationService.getAdultFemaleLiteracy(
            country,
            dateRange
          );
        ...
        case WorldBankApiV2Indicators.LIFE_EXPECTANCY:
          data = await this._populationService.getLifeExpectancy(
            country,
            dateRange
          );
        ...
        case WorldBankApiV2Indicators.ADULT_FEMALE_SURVIVAL_TO_65:
          data = await this._populationService.getFemaleSurvivalToAge65(
            country,
            dateRange
          );
        ...
        case WorldBankApiV2Indicators.ADULT_MALE_SURVIVAL_TO_65:
          data = await this._populationService.getMaleSurvivalToAge65(
            country,
            dateRange
          );
          title = "Adult male survive to age 65";
          break;
        default:
          throw new UnreachableCaseError(indicator);
      }

      const dataLabel = `${title} in ${country.name} for the ${
        chartFormDetails.fromYear
      }-${chartFormDetails.toYear} time period`;
